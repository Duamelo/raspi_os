...
static gpio_t* GPIO_BASE = (gpio_t*)P_GPIO_BASE;
...
{ 
        ...
        if ( (pin / 32) == 0  )
            return gpio_call(pin, value, GPIO_BASE->GPSET0, 1, GPIO_MAX_PIN);
    
        else
            return gpio_call(pin, value, GPIO_BASE->GPSET1, 1, GPIO_MAX_PIN);
...
{ 
        ...
        if ( (pin / 32) == 0  )
            return gpio_call(pin, value, GPIO_BASE->GPCLR0, 1, GPIO_MAX_PIN);
    
        else
            return gpio_call(pin, value, GPIO_BASE->GPCLR1, 1, GPIO_MAX_PIN);
...
{ 
    return gpio_call(pin, pull, GPIO_BASE->GPPUD, 2, GPIO_MAX_PIN); 
}
...
unsigned int gpio_get_value( unsigned int pin)
{
    ...
    {
        case 0:
            result =  &(GPIO_BASE->GPLEV0) & ( 1 << pin ) ;
            break;

        case 1:
            result = ( &(GPIO_BASE->GPLEV1) & ( 1 << ( pin - 32 ) ) );
            break;
